...
# -*- coding: utf-8 -*-
"""
Python Version Checker for CredTech XScore
This script helps identify available Python versions and their capabilities
"""
...
def check_current_python():
    ...
    if "f-strings" in features:
        print("   ✓ Compatible with enhanced features")
        return True
    else:
        print("   ✗ NOT compatible with enhanced features (requires Python 3.6+)")
        return False
...
def check_python_3_8_executable():
    ...
    if os.path.exists(python_38_path):
        print("   ✓ Python 3.8.10 executable found")
        ...
        try:
            ...
            if result.returncode == 0:
                print("   ✓ Python 3.8.10 is working: {}".format(result.stdout.strip()))
                return True
            else:
                print("   ✗ Python 3.8.10 failed: {}".format(result.stderr.strip()))
                return False
        except subprocess.TimeoutExpired:
            print("   ⚠️  Python 3.8.10 timed out (may need installation)")
            return False
        except Exception as e:
            print("   ✗ Error running Python 3.8.10: {}".format(e))
            return False
    else:
        print("   ✗ Python 3.8.10 executable not found")
        return False
        ...
        if os.path.exists(path):
            try:
                ...
                if result.returncode == 0:
                    ...
                    print("   ✓ Found: {} - {}".format(path, version))
                    found_pythons.append((path, version))
                else:
                    print("   ⚠️  Found but not working: {}".format(path))
            except Exception as e:
                print("   ⚠️  Found but error: {} - {}".format(path, e))
...
def test_enhanced_features():
    ...
    try:
        ...
        try:
            ...
            print("   ✗ Enhanced logging import failed: {}".format(e))
        
        try:
            import src.utils.monitoring
            print("   ✓ Monitoring module imported successfully")
        except Exception as e:
            print("   ✗ Monitoring import failed: {}".format(e))
        
        try:
            import src.config.dashboard_config
            print("   ✓ Dashboard config imported successfully")
        except Exception as e:
            print("   ✗ Dashboard config import failed: {}".format(e))
        
        return True
        
    except Exception as e:
        print("   ✗ Enhanced features test failed: {}".format(e))
        return False
...
def main():
    ...
    if current_ok:
        print("✓ Current Python is compatible with enhanced features!")
    else:
        print("✗ Current Python 3.4 is NOT compatible with enhanced features")
    
    if python38_ok:
        print("✓ Python 3.8.10 executable is working")
    else:
        print("✗ Python 3.8.10 executable needs installation")
    
    if system_pythons:
        print("✓ Found {} additional Python installation(s)".format(len(system_pythons)))
    else:
        print("✗ No additional Python installations found")
    
    if enhanced_ok:
        print("✓ Enhanced features can be imported")
    else:
        print("✗ Enhanced features cannot be imported (Python version issue)")
    
    print("\n" + "=" * 70)
    
    if current_ok or python38_ok or system_pythons:
        print("🎉 You have options to run the enhanced features!")
    else:
        print("⚠️  You need to install Python 3.6+ to use enhanced features")
