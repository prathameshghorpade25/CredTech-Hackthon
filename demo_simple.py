...
# -*- coding: utf-8 -*-
"""
Simple Demo Script for CredTech XScore
Compatible with Python 3.4 - No external dependencies
"""
...
def print_header():
    """Print a nice header"""
    print("=" * 60)
    print("CredTech XScore - Simple Demo")
    print("=" * 60)
    print()
...
def check_data_files():
    ...
    for file_path in data_files:
        if os.path.exists(file_path):
            ...
            print("   OK: {} exists ({} bytes)".format(file_path, file_size))
        else:
...
def show_sample_data():
    ...
    try:
        ...
        if os.path.exists('data/processed/combined_data.csv'):
            print("   Combined Data:")
            with open('data/processed/combined_data.csv', 'r') as f:
                lines = f.readlines()
                print("   Total lines: {}".format(len(lines)))
                if len(lines) > 0:
                    print("   Header: {}".format(lines[0].strip()))
                    if len(lines) > 1:
                        print("   First data row: {}".format(lines[1].strip()))
...
def show_project_info():
    ...
    try:
        ...
        print("   Project size: {} bytes ({:.1f} MB)".format(total_size, total_size / (1024*1024)))
...
def show_python_info():
    """Show Python version and capabilities"""
...
def main():
    ...
    checks = [
        ...
        ("Python Environment", show_python_info)
    ]
    ...
    if passed == total:
        ...
        print("\nWARNING: Some checks failed. Check the output above.")
